  ...
  try {
    ...
    if (subcmd === 'on') {
      ...
      return await sock.sendMessage(chatId, {
        text: `🤖 *Popkid AI Activated!*\n\nNow I’ll chat with you naturally.\n\nType anything to begin 💬\n\n> *${config.PREFIX}chatbot off* — to stop me`
      }, { quoted: m });
    }
    ...
    if (global.chatbotEnabled[sender]) {
      ...
      // only reply if mentioned or replied in group
      if (isGroup) {
...
💬 Current: ${userMessage}`;
      ...
      try {
        ...
        console.log('Chatbot API error:', err.message);
      }
    }

  } catch (err) {
    console.error('Chatbot case error:', err);
    await sock.sendMessage(m.key.remoteJid, { text: '⚠️ Error in chatbot function.' }, { quoted: m });
  }
  break;
}
                // NEWS COMMAND
