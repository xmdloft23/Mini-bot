...
`
    ...
    break;
                ...
                //video.js
                // ==========================================
// 🎬 VIDEO DOWNLOAD COMMAND (POPkid XMD Style)
// ==========================================
case 'video': {
  try {
    const text = (msg.message.conversation || msg.message.extendedTextMessage?.text || '').trim();
    ...
    // 🔎 Step 1: Search YouTube videos using Malvin or fallback API
    const searchUrl = `https://apis.davidcyriltech.my.id/search/youtube?query=${encodeURIComponent(q)}`;
    const searchResponse = await fetch(searchUrl);
    const searchData = await searchResponse.json();
    const videoData = searchData.videos?.[0];
    if (!videoData) {
      await socket.sendMessage(sender, { text: "⚠️ No video found for your search!" });
      return;
    }

    // 🔽 Step 2: Try to get a downloadable link
    let downloadInfo;
    try {
      const res1 = await fetch(`https://apis.davidcyriltech.my.id/download/ytmp4?url=${encodeURIComponent(videoData.url)}`);
      const data1 = await res1.json();
      if (!data1?.result?.download_url) throw new Error("Primary failed");
      downloadInfo = {
        title: data1.result.title,
        thumbnail: data1.result.thumbnail,
        download_url: data1.result.download_url,
        quality: data1.result.quality || "Unknown"
      };
    } catch {
      const res2 = await fetch(`https://iamtkm.vercel.app/downloaders/ytmp4?url=${encodeURIComponent(videoData.url)}`);
      const data2 = await res2.json();
      if (!data2?.data?.url) throw new Error("Fallback failed");
      downloadInfo = {
        title: data2.data.title || videoData.title,
        thumbnail: videoData.thumbnail,
        download_url: data2.data.url,
        quality: "Unknown (fallback)"
      };
    }

    // 🖼️ Step 3: Send video preview
    const previewCaption = `
╭────「 *🎬 Video Preview* 」────╮
│ 📌 *Title:* ${downloadInfo.title}
│ ⏱️ *Duration:* ${videoData.timestamp || "N/A"}
│ 👁️ *Views:* ${videoData.views || "N/A"}
