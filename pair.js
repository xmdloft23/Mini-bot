...
`
  ...
  try {
    ...
    if (!q) {
      await socket.sendMessage(sender, { 
        text: `
🚫 *Please enter a video name to search!*

Example:
> ${config.PREFIX}video faded
`
      });
      return;
    }

    await socket.sendMessage(sender, { text: `🔍 *Searching for video:* _${q}_ ...` });

    // 🔗 Call Malvin API
    const apiUrl = `https://malvin-api.vercel.app/search/youtube?q=${encodeURIComponent(q)}`;
    const apiResponse = await fetch(apiUrl);
    const data = await apiResponse.json();

    // 🧩 Adjust this depending on response format
    const results = data.results || data.videos || data; 
    if (!results || results.length === 0) {
      await socket.sendMessage(sender, { 
        text: `⚠️ *No video found for:* ${q}`
      });
      return;
    }

    const video = results[0]; // pick first result
    const title = video.title || "Untitled Video";
    const channel = video.channel || video.channelName || "Unknown";
    const views = video.views || "N/A";
    const duration = video.duration || "N/A";
    const url = video.url || video.link || video.videoUrl;
    const thumbnail = video.thumbnail || video.thumbnailUrl || video.image;
