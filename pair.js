...
`
    ...
    break;
                ...
                //
                // 💬 Chatbot Command (Popkid MD Mini)
case 'chatbot': {
    try {
        ...
        if (arg === "on") {
            ...
            await socket.sendMessage(sender, { 
                text: `
🤖 *Popkid AI Chatbot Activated!*

You can now talk naturally with *Popkid AI*.
Example:
> hi popkid

Type *${config.PREFIX}chatbot off* to deactivate.
`
            });
            return;
        }
        ...
        if (arg === "off") {
            ...
            await socket.sendMessage(sender, { 
                text: `
🚫 *Popkid AI Chatbot Deactivated.*

Bot will now ignore chat messages.

Type *${config.PREFIX}chatbot on* to activate again ✨
`
            });
            return;
        }

        // 💬 Normal user messages handled here
        if (global.chatbotEnabled[sender]) {
            const userMessage = text.replace(`${config.PREFIX}chatbot`, "").trim() || "Hello";

            if (!global.userChats[sender]) global.userChats[sender] = [];

            const history = global.userChats[sender].slice(-5).join("\n");

            const prompt = `
You are Popkid AI, a smart and friendly WhatsApp assistant created by Popkid from Kenya.
...
`;
            ...
            const botReply = (data?.status === 200 && data?.success && data?.result?.prompt)
                ? data.result.prompt
                : "*⚠️ Sorry, Popkid AI failed to respond right now.*";

            // 🗂️ Update chat memory
...
Turn it on to chat with Popkid AI:
